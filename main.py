...
MIN_WITHDRAWAL_AMOUNT = 10  # Minimum withdrawal amount in TMT
...
TEXTS = {
    "ru": {
        ...
        "admin_user_blocked": "✅ Пользователь {user_name} (ID: {user_id}) заблокирован",
        "admin_user_unblocked": "✅ Пользователь {user_name} (ID: {user_id}) разблокирован",
        "order_reminder": "⏰ Напоминание о заказе!\n\n📋 Заказ #{order_id}: {title}\n⚠️ Заказ ожидает завершения более {days} дней",
    },
    "tm": {
        ...
        "user_blocked": "🚫 Siziň hasabyňyz petiklendi. Goldaw bilen habarlaşyň.",
        "admin_user_blocked": "✅ Ulanyjy {user_name} (ID: {user_id}) petiklendi",
        "admin_user_unblocked": "✅ Ulanyjy {user_name} (ID: {user_id}) petikden aýryldy",
        "order_reminder": "⏰ Sargyt ýatlatmasy!\n\n📋 Sargyt #{order_id}: {title}\n⚠️ Sargyt {days} günden köp tamamlanmaga garaşýar",
    }
}
...
def format_service_text(service: dict, lang: str = "ru", show_contact: bool = True) -> str:
    ...
    rating = get_user_average_rating(service['user_id']) if user else 0.0
...
👤 <b>{"Фрилансер" if lang == "ru" else "Frilanser"}:</b> {escape_html(user.get('first_name', 'Unknown') if user else 'Unknown')}
📱 <b>Username:</b> {username}
🆔 <b>ID:</b> <code>{service['user_id']}</code>
⭐ <b>{"Рейтинг" if lang == "ru" else "Reýting"}:</b> {rating:.1f}/5.0
"""
        ...
        if isinstance(event, (Message, CallbackQuery)):
            ...
            if user_id:
                ...
                if is_blocked(user_id):
                    ...
                    blocked_text = get_text("user_blocked", lang)
...
async def admin_manage_users(callback: CallbackQuery):
    ...
    for user in all_users[:10]:
        ...
        blocked_emoji = "🚫" if user.get('is_blocked') else ""
        username_text = f"@{user.get('username')}" if user.get('username') else "без username"
        balance = get_user_balance(user['id'])
        
        text += f"{role_emoji}{blocked_emoji} <b>{escape_html(user.get('first_name', 'Unknown'))}</b> ({username_text})\n"
    ...
    keyboard.append([InlineKeyboardButton(text="🚫 Блокировка пользователей", callback_data="admin_block_users")])
    keyboard.append([InlineKeyboardButton(text="◀️ Назад", callback_data="admin_back")])
...
async def withdrawal_amount_received(message: Message, state: FSMContext):
    ...
    try:
        ...
        if amount < MIN_WITHDRAWAL_AMOUNT:
            await message.answer(get_text("withdraw_min_amount", lang))
            return
...
async def confirm_withdrawal(callback: CallbackQuery, state: FSMContext):
    ...
    admin_text += get_text("admin_withdrawal_info", lang).format(
        ...
        commission=format_price(temp_withdrawal['commission']),
        final_amount=format_price(temp_withdrawal['final_amount']),
...
# BLOCK USER HANDLERS
# =============================================================================
@router.callback_query(F.data == "admin_block_users")
async def admin_block_users(callback: CallbackQuery):
    """Show user blocking management"""
    user_id = callback.from_user.id
    ...
    text = "🚫 <b>Управление блокировками</b>\n\n"
    keyboard = []
    ...
    for user in all_users[:15]:
        ...
        text += f"{role_emoji}{blocked_emoji} <b>{escape_html(user.get('first_name', 'Unknown'))}</b> ({username_text})\n"
        text += f"ID: {user['id']} | {'ЗАБЛОКИРОВАН' if blocked else 'Активен'}\n\n"
        
        if blocked:
            keyboard.append([
                InlineKeyboardButton(
                    text=f"✅ Разблокировать {user.get('first_name', 'User')[:15]}",
            ...
            keyboard.append([
                InlineKeyboardButton(
                    text=f"🚫 Заблокировать {user.get('first_name', 'User')[:15]}",
                    callback_data=f"admin_block_{user['id']}"
                )
            ])
...
async def admin_block_user_callback(callback: CallbackQuery):
    ...
    await callback.answer(f"🚫 Пользователь {user_name} заблокирован")
    
    # Notify user about block
    lang = target_user.get('language', 'ru')
    try:
        await callback.bot.send_message(target_user_id, get_text("user_blocked", lang))
    except Exception as e:
        logger.error(f"Failed to notify blocked user {target_user_id}: {e}")

    # Refresh the list
    await admin_block_users(callback)

@router.callback_query(F.data.startswith("admin_unblock_"))
async def admin_unblock_user_callback(callback: CallbackQuery):
    """Unblock user"""
    admin_id = callback.from_user.id
    ...
    user_name = target_user.get('first_name', 'Unknown')
    await callback.answer(f"✅ Пользователь {user_name} разблокирован")
...
# ORDER REMINDERS
# =============================================================================
async def send_order_reminders(bot: Bot):
    """Send reminders for orders that are in progress for too long"""
    try:
        for order in orders_db.values():
            ...
            if days_passed >= 3 and not order.get('reminder_sent'):
                # Notify client
                client = get_user(order['client_id'])
                if client:
                    lang = client.get('language', 'ru')
                    reminder_text = get_text("order_reminder", lang).format(
                        order_id=order['id'],
                        title=order['title'],
                        days=days_passed
                    )
                    try:
                        await bot.send_message(order['client_id'], reminder_text)
                    except Exception as e:
                        logger.error(f"Failed to send reminder to client {order['client_id']}: {e}")
                
                # Notify freelancer
                freelancer_id = order.get('selected_freelancer')
                if freelancer_id:
                    freelancer = get_user(freelancer_id)
                    if freelancer:
                        lang = freelancer.get('language', 'ru')
                        reminder_text = get_text("order_reminder", lang).format(
                            order_id=order['id'],
                            title=order['title'],
                            days=days_passed
                        ...
                        try:
                            await bot.send_message(freelancer_id, reminder_text)
                        except Exception as e:
                            logger.error(f"Failed to send reminder to freelancer {freelancer_id}: {e}")
...
async def reminder_scheduler(bot: Bot):
    ...
    while True:
        ...
        await send_order_reminders(bot)
